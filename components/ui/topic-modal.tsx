      ...
      <DialogContent className="bg-[#001A99] dark:bg-[#111a2e] border-white/20 dark:border-slate-700/50 text-white sm:max-w-2xl p-6 rounded-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center gap-2 mb-1">
            ...
            {phaseTitle && (
              <span className="text-xs text-white/50">{phaseTitle}</span>
            )}
          </div>
          ...
          <DialogDescription className="text-xs text-white/70 leading-relaxed mt-1">
            {topic.description}
          </DialogDescription>
        </DialogHeader>

        {/* Action Bar: Toggle Complete & SRS Review Picker */}
        <div className="my-4 p-4 rounded-2xl bg-white/10 dark:bg-white/5 border border-white/10 flex flex-wrap items-center justify-between gap-4">
          <button
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-white/60 font-semibold">SRS Review:</span>
            {[3, 7, 14].map((days) => (
              <button
                key={days}
                onClick={() => handleSetSRSReview(days)}
                className="px-3 py-1 rounded-full text-[11px] font-bold bg-white/10 hover:bg-[#CCFF00] hover:text-black text-white/80 transition-colors"
              >
                +{days}d
              </button>
            ))}
        ...
        {nextReviewDate && (
          <div className="flex items-center justify-between text-xs bg-purple-500/20 border border-purple-500/40 p-3 rounded-xl text-purple-300">
        ...
        <div className="space-y-3 mt-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1 bg-white/10 p-1 rounded-full text-[10px]">
              <button
                onClick={() => setActiveTab("edit")}
                className={`px-3 py-1 rounded-full font-semibold ${
                  activeTab === "edit" ? "bg-[#CCFF00] text-black" : "text-white/60"
                }`}
              >
                Edit
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-full font-semibold ${
                  activeTab === "preview" ? "bg-[#CCFF00] text-black" : "text-white/60"
                }`}
              >
                Preview
              </button>
